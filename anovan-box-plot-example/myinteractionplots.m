function  [H, ax, BigAx]  = myinteractionplots(y,group,varargin)
...
%   vector, string array, character matrix, or single-column cell array of
%   strings. GROUP can also be a matrix whose columns represent different
%   grouping variables.  Each grouping variable must have the same number
%   of rows as Y.  The number of grouping variables must be greater than 1.
%
...
%   variable names are printed on the diagonal of the plot matrix. The plot
%   at off-diagonal position (i,j) is the interaction of the two variables
...
%
%       Parameter    Value
%       'varnames'   Grouping variables names in a character matrix, a
%                    string array, or cell array of character vectors, one
%                    per grouping variable (default names are 'X1', 'X2',
%                    ...)
...
%   window, a matrix AX of handles to the subplot axes, and a handle BIGAX
%   to the big (invisible) axes framing the subplots.
%
%   Example:
%     Display interaction plots for data with four 3-level factors named
%     'A', 'B','C', and 'D'.
%        y = randn(1000,1); %response group = ceil(3*rand(1000,4)); %four
%        3-level factors
    ...
    for i=rows:-1:1,
        for j=cols:-1:1,
            ...
            if  i~=j   % off- diagonal are filled with interaction plots
                ...
                % make the legend
                legh = legend(lentext,'FontSize',fontsize,'location','northoutside','orientation', 'horizontal');
                % place the legend to the very right
                legpos = get(legh, 'position');
                legpos(1) = legx;
                %set(legh, 'position',legpos)
                set(gca, 'FontSize', fontsize)
                xlabel(varnames{anotheridx})
                ytickformat('%.2f')
            end
        end
    end
...
for i = 1:num2
    ...
    handle(i) = plot(1:num1,matrixdata(i,:),linespec,'color',colors(idxcolor,:), 'linewidth',2);
end;
hold off
axis tight
